$(function () {
  ...
  function loadInitialHospitals() {
    ...
    $.ajax({
      ...
      contentType: "application/x-www-form-urlencoded;charset=EUC-KR",
        ...
        .done(function (data) {
          result.empty();
          const hospitalsToShow = data.slice(0, 5); // First 5 items
          $.each(hospitalsToShow, function (_, hospital) {
            result.append(`
              ...
              data-facility_name="${hospital.facility_name}" data-operate_time="${hospital.operate_time}"
              data-day_off="${hospital.day_off}" data-tel="${hospital.tel}"
              data-road_address="${hospital.road_address}" data-parking="${hospital.parking}"
              data-pet_size="${hospital.pet_size}" data-image="${hospital.image}">
                ${hospital.facility_name}
                <small class="ms-auto">${hospital.road_address}</small>
              </button>
            `);
          });
          ...
          showLocation(lat, lng, hospital);
        });

        // 첫 번째 병원 버튼 클릭 이벤트 트리거
  ...
  async function showLocation(latitude, longitude, hospital) {
    if (!map || !service) {
      console.error("지도 객체 또는 PlacesService 객체가 초기화되지 않았습니다.");
      return;
    }
    ...
    const infoWindow = new google.maps.InfoWindow({
      content: `<div><strong>${hospital.facility_name}</strong><br>운영시간: ${hospital.operate_time}</div>`,
    });
    ...
    getPlaceDetails(latitude, longitude, hospital.facility_name);
  }
  ...
  function updateHospitalInfo(hospital) {
    ...
    if (hospital.image) {
      ...
      $("#hospitalImage").attr("src", "../../images/section_hospitalInfo.png").addClass("img-fluid");
    }
  }
